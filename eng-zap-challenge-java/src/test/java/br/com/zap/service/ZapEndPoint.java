...
public class ZapEndPoint {
	...
	public void testReturn200() {
		...
		ResponseEntity<RealEstateAdWrapper> resp = new RestTemplate().getForEntity(url + port + "/zap",
	...
	public void testReturn404() throws ClientProtocolException, IOException {
		...
		HttpUriRequest request = new HttpGet(url + port + "/zap?page=" + (resp.getBody().getTotalPage() + 1) + "&size="
	...
	public void testReturnPaginator() throws ClientProtocolException, IOException {
		...
		HttpUriRequest request = new HttpGet(url + port + "/zap?page=" + (resp.getBody().getTotalPage()) + "&size="
				+ resp.getBody().getPageSize());
